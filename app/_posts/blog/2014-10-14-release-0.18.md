...
Every legacy methods (`this.write`, `this.copy`, [etc](http://hardwarelivreusp.org/generator/actions.html)) have been back ported to use this new system. So updating to 0.18.0 should magically improve your generator.
...
- Tests [using `helpers.run()`](http://hardwarelivreusp.org/authoring/testing.html) won't output to the console keeping the test output clean. Every log method is now a `sinon.stub()` making it easy to assert your generator outputted the correct content.
...
Every install method will be registered on the [`install` run queue](http://hardwarelivreusp.org/authoring/running-context.html). Using `this.async()` as a callback will create a deadlock.
